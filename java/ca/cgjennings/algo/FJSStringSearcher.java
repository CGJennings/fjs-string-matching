 ...
 * It is guaranteed to run in O(n) time, but usually runs in much less.
 * This implementation uses a simple hashing strategy to avoid paying a penalty
 * for Unicode's large alphabet size, with the consequence that this
 * implementation will be slower for certain large texts with many hash
 * collisions than an unhashed FJS implementation (very rare in practice).
 *
